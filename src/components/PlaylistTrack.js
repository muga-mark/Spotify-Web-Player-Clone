...
function PlaylistTrack({ track, date_added }) {
  ...
  useEffect(() => {
    ...
    if (date_added) {
      ...
      var event = new Date(
        Date.UTC(date_split[0], date_split[1], date_split[2], 0, 0, 0)
      );
  ...
  return (
    <div className="playlist-track">
      ...
      <div className="playlist-track-center">
        <div className="playlist-track-title-container">
          ...
          <div className="playlist-track-title">
            <span className="playlist-track-song-title playlist-track-text playlist-track-text--bold  playlist-track-text--white">
              {track.name}
            </span>
            <span
              className="playlist-track-artist playlist-track-text"
              id="track"
            >
      ...
      <span className="number-sign playlist-track-text">{duration}</span>
    </div>
  );
}
